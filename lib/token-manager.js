...
TokenManager.prototype = {
	...
	getTokensJWTGrant(type, id, options) {
		...
		return this.getTokens(params, options)
			.catch(err => this.retryJWTGrant(claims, jwtOptions, keyParams, params, options, err));
	},
	 ...
	 * @param {Object} [jwtOptions] - Options for the signature
	 * @param {Object} keyParams - Key parameters object
	 * @param {Object} params - should contain all params expected by Box OAuth2 token endpoint
	 * @param {TokenRequestOptions} [options] - Sets optional behavior for the token grant
	 * @param {Error} err - Error from the previous JWT request
	 * @returns {Promise<TokenInfo>} Promise resolving to the token info
	 */
	retryJWTGrant(claims, jwtOptions, keyParams, params, options, err) {
		this.numRetries = this.numRetries || 0;

		if (this.numRetries < this.config.numMaxRetries && isJWTAuthErrorRetryable(err)) {
			var retryTimeout;
			this.numRetries += 1;
			...
			if (this.config.retryStrategy) {
				...
				retryTimeout = this.getRetryTimeout(this.numRetries, this.config.retryIntervalMS);
			}
			...
			claims.exp = Math.floor(Date.now() / 1000) + this.config.appAuth.expirationTime;
			...
			return this.delay(retryTimeout).then(function() {
				return this.getTokens(params, options)
					.catch(error => this.retryJWTGrant(claims, jwtOptions, keyParams, params, options, error));
			});
		}
	 ...
	 *
	 * @param {int} t - Time to delay
	 * @returns {Promise<TokenInfo>} Promise resolving to the delay function
	 */
	delay(t, v) {
