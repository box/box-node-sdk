...
TokenManager.prototype = {
	 ...
	 * @param {string} scope - The scope of the new access token
	 * @param {?string} resource - The absolute URL of an API resource to restrict the new token to
	 * @param {?Object} delegateInfo - Information about the delegated user, if the token should be issued under another user
	 * @param {string} delegateInfo.id - The ID of the user who the new token should belong to
	 * @param {string} [delegateInfo.name] - The full name of the user who the new token should belong to
	 * @param {Function} callback - Passed the new token data if successful, error otherwise
	 * @returns {void}
	 */
	exchangeToken: function(accessToken, scope, resource, delegateInfo, callback) {
		...
		if (delegateInfo && delegateInfo.id) {
			var claims = {
			...
			var options = {
				algorithm: 'none',
				audience: 'https://api.box.com/oauth2/token',
				subject: delegateInfo.id,
				issuer: this.config.clientID,
				jwtid: uuid.v4(),
				expiresIn: 60
			};
			if (delegateInfo.name) {
				claims.name = delegateInfo.name;
			}

			try {
				params.actor_token = jwt.sign(claims, null, options);
		...
		this.getTokens(params, null, callback);
	},
