...
class PagingIterator {
	...
	constructor(response, client) {
		...
		var queryIndex = response.request.href.indexOf('?');
		var href = queryIndex > 0 ? response.request.href.substring(0, queryIndex) : response.request.href;
		this.options = {
			headers: response.request.headers,
			qs: querystring.parse(response.request.uri.query)
		};

		// qs.parse() makes everything a string, ensure the limit is numeric
