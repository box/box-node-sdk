...
class PagingIterator {
	...
	static isIterable(response) {
		return response.request && response.request.method === 'GET' && Array.isArray(response.body.entries);
	}
	 ...
	 * When the well-known symbol isn't available, use an arbitrary new one to avoid attaching this
	 * method as PagingIterator#undefined()
	 *
	 * @returns {PagingIterator} This iterator
	 */
	get [Symbol.asyncIterator || Symbol.for('boxAsyncIterator')]() {
		return this;
	}
}
