...
class PagingIterator {
	...
	static isIterable(response) {
		var isGetRequest = (response.request.method === 'GET'),
			hasEntries = Array.isArray(response.body.entries),
			notEventStream = !response.body.next_stream_position;

		return isGetRequest && hasEntries && notEventStream;
	}
