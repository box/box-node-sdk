...
/**
 * The number of ms to wait before retrying after an error
 * @const {int}
 */
var RETRY_DELAY = 1000;

/**
 * Max number of IDs to track for deduplication before cleaning
 * @const {int}
 */
var MAX_DEDUP_SIZE = 5000;

/**
 * Amount of time that must elapse between two successive calls to get more events,
 * which is necessary to prevent against scenarios where there is a constant flow of events
 * tripping API rate limits.
 * @const {int}
 */
var FETCH_EVENTS_RATE_LIMIT = 1000;
...
EventStream.prototype.getLongPollInfo = function() {
	...
	return this._client.events.getLongPollInfo()
		...
		.catch(err => {
			...
			if (!err.authExpired) {
				setTimeout(() => this.getLongPollInfo(), RETRY_DELAY);
			}
		});
};
...
EventStream.prototype.fetchEvents = function() {
	...
	return this._rateLimiter.then(() => this._client.events.get(eventParams)
			.then(events => {
				...
				this._rateLimiter = Promise.delay(FETCH_EVENTS_RATE_LIMIT);
				...
				if (Object.keys(this._dedupHash).length >= MAX_DEDUP_SIZE) {
