...
EnterpriseEventStream.prototype.fetchEvents = function(callback) {
	...
	if (this._options.startDate) {
		options.createdAfter = this._options.startDate;
	}

	if (this._options.endDate) {
		options.createdBefore = this._options.endDate;
	}

	if (this._options.eventTypeFilter) {
		options.eventTypeFilter = this._options.eventTypeFilter;
	}
...
EnterpriseEventStream.prototype._read = function() {
	if (this._eventBuffer.length > 0) {
		...
		// After all of the events have been consumed, notify the reader of the new stream position.
		// Stream readers can respond to the 'chunk' event to persist the stream position.
		const streamPosition = this.getStreamPosition();

		// Capture the current stream position here, but emit the event after the current 'data' event is done.
		setImmediate(function() {
			self.emit('chunk', streamPosition);
		});
