...
/**
 * Read and set the stream position and start date from a file.
 *
 * @param {string} file - A filename or file descriptor
 * @param {Function} callback - Passed the stream position and start date
 * @returns {void}
 */
EnterpriseEventStream.prototype.readStreamPositionFromFile = function(file, callback) {
	const self = this;
	...
	fs.readFile(file, function(err, data) {
		...
		// We need to read both streamPosition and startDate, since streamPosition will be null until
		// the first set of events is returned from Box.
		const json = JSON.parse(data);
		self._streamPosition = json.streamPosition;
		self._options.startDate = json.startDate;
		self._options.endDate = json.endDate;
		self._options.eventTypeFilter = json.eventTypeFilter;
...
/**
 * Write the stream position and start date to a file.
 *
 * @param {string} file - A filename or file descriptor
 * @param {Function} callback - Called when done
 * @returns {void}
 */
EnterpriseEventStream.prototype.writeStreamPositionToFile = function(file, callback) {
	// We need to write both streamPosition and startDate, since streamPosition will be null until
	// the first set of events is returned from Box.
...
EnterpriseEventStream.prototype.fetchEvents = function(callback) {
	...
	if (this._options.eventTypeFilter) {
		params.event_type = this._options.eventTypeFilter;
	}
	...
	this._client.events.get(params, function(err, result) {
		if (err) {
			...
			// console.log(`Fetched events from ${options.streamPosition || options.createdAfter} (limit ${options.limit}) => ${result.entries.length} events (next ${result.next_stream_position})`);
...
EnterpriseEventStream.prototype._read = function() {
	...
	this.fetchEvents(function(err, events) {
		...
		// Stream readers can respond to the 'streamPosition' event to persist the stream position.
		self.emit('streamPosition', self.getStreamPosition());
