...
PersistentSession.prototype._refreshTokens = function(options) {
	...
	if (!this._refreshPromise) {
		...
		this._refreshPromise = this.tokenManager.getTokensRefreshGrant(this._tokenInfo.refreshToken, options)
			.catch(err => {
...
PersistentSession.prototype.getAccessToken = function(options) {
	// If our tokens are not fresh, we need to refresh them before passing the access token to the callback
...
PersistentSession.prototype.revokeTokens = function(options) {
	return this.tokenManager.revokeTokens(this._tokenInfo.refreshToken, options);
};
