...
APIRequest.prototype.execute = function(callback) {
	...
	if (this._callback) {
		...
		// this.config.request.formData.content.knownLength = 5;
		this.config.request.formData.knownLength = 50;
		this.request = request(this.config.request, this._handleResponse.bind(this));
		// console.log(this.config.request.formData.content.value);
	} else {
		// console.log(this.config.request);
