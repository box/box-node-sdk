...
APIRequest.prototype._handleResponse = function(err, response) {
	...
	if (err) {
		...
		// If our APIRequest instance is retryable, attempt a retry. Otherwise, finish and propagate the error. Doesn't retry when the request is for JWT authentication.
...
APIRequest.prototype._retry = function(err) {
	...
	if (this.numRetries < this.config.numMaxRetries) {
		...
		} else if (err.response.headers.hasOwnProperty('retry-after')) {
