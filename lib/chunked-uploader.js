...
class ChunkedUploader extends EventEmitter {
	...
	constructor(client, uploadSessionInfo, file, size, options) {
		...
		this._client = client;
		this._sessionID = uploadSessionInfo.upload_session_id;
	...
	_getNextChunk(callback) {
		...
		if (this._file) {
			...
			buf = this._stream.read(this.partSize);
		...
		let chunk = new Chunk(this._client, this._sessionID, buf, this._position, this._size);
	...
	_commit() {
		...
		this._isStarted = false;
		this._client.files.commitUploadSession(this._sessionID, hash, this._chunks.map(c => c.getData()), null, (err, file) => {
