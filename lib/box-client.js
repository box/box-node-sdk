...
BoxClient.prototype.ITERATOR = PagingIterator.SYMBOL;
...
/**
 * Get an iterator for a given response
 *
 * @param {Object} response The raw API response object
 * @returns {PagingIterator} The iterator for the response
 * @private
 */
BoxClient.prototype._getIterator = function(response) {
	return new PagingIterator(response, this);
};
	...
	return function(err, response) {
		...
		if (response.statusCode >= HTTP_STATUS_CODE_SUCCESS_BLOCK_RANGE[0]
			...
			if (PagingIterator.isIterable(response)) {
				...
				if (self._useIterators) {
					callback(null, self._getIterator(response));
					return;
				}

				// Make the return value async-iterable on versions that support it
				response.body[Symbol.asyncIterator || PagingIterator.SYMBOL] = self._getIterator.bind(self, response);

				// Ensure the "legacy" way of accessing the iterator is forwards-compatible
				response.body[PagingIterator.SYMBOL] = self._getIterator.bind(self, response);
			}
			...
			ret = ret.then(response => {
				...
				if (response.statusCode >= HTTP_STATUS_CODE_SUCCESS_BLOCK_RANGE[0]
					...
					if (PagingIterator.isIterable(response)) {
						...
						if (self._useIterators) {
							return self._getIterator(response);
						}
						// Make the return value async-iterable on versions that support it
						response.body[Symbol.asyncIterator || PagingIterator.SYMBOL] = self._getIterator.bind(self, response);
