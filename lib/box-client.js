 ...
 * @returns {Promise} Promise resolving to the response or client if in batch mode
 * @private
 */
BoxClient.prototype._makeRequest = function(params, cb) {
	...
	if (this._batch) {
		...
		promise = Promise.fromCallback(function(callback) {
			...
			self._session.getAccessToken(self._tokenOptions, function(err, accessToken) {
				...
				self._requestManager.makeRequest(params, function(responseErr, response) {
					callback(null, responseErr, response);
				});
			});
		}, {multiArgs: true});
	}
	...
	return promise
		.then(args => Promise.fromCallback(callback => {
...
BoxClient.prototype.execute = function(callback) {
	...
	return this.post('/batch', params)
		.then(res => {
			...
			responses.map(formatResponseForBatch).forEach((response, index) => {
				batch[index].resolve([null, response]);
			});
