...
BoxClient.prototype._makeRequest = function(params, cb) {
	...
	var promise = Promise.fromCallback(function(callback) {
		...
		self._session.getAccessToken(this._ipOptions, function(err, accessToken) {
			// Handle Error
			if (err) {
				callback(err);
				return;
...
BoxClient.prototype.setIPs = function(ips) {
	...
	this.setCustomHeader(HEADER_XFF, this._ipOptions);

	this._ipOptions = {};
	this._ipOptions.ip = validIPs;
};
...
BoxClient.prototype.revokeTokens = function(callback) {
	...
	return Promise.fromCallback(cb => this._session.revokeTokens(this._ipOptions, cb))
...
BoxClient.prototype.exchangeToken = function(scopes, resource, callback) {
	...
	return Promise.fromCallback(cb => this._session.exchangeToken(scopes, resource, this._ipOptions, cb))
		.catch(err => errors.unwrapAndThrow(err))
		.asCallback(callback);
};
