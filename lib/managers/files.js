 ...
 *
 * @param {string} fileID - Box ID of the file being requested
 * @param {RepresentationsOptions[]} representationTypes - The x-rep-hints value the application should create a
 *    representation for
 * @param {string} [options.contentDispositionType] - Set to either 'inline' for the file to be downloaded from the
 *    browser or 'attachment' which opens the file in another tab
 * @param {string} [options.fileName] - If defined this will name the downloaded representation file, if not the name will
...
Files.prototype.getRepresentations = function(fileID, representationTypes, options, callback){
  ...
  object.assign(params.header, options)
  var apiPath = urlPath(BASE_PATH, fileID);
  return this.client.wrapWithDefaultHandler(this.client.get)(apiPath, params, callback);
}
