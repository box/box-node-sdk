...
function pollRepresentationInfo(client, infoURL, options) {
	...
	return this.getRepresentationInfo(fileID, representationType)
		.then(reps => {
			...
			switch (repInfo.status.state) {
				...
				case 'none':
				case 'pending':
					return pollRepresentationInfo(this.client, repInfo.info.url, options);
					break;
				default:
		...
		.then(assetURLTemplate => {
			var url = assetURLTemplate.replace(ASSET_PATH, options.assetPath);
