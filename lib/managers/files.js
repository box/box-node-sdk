	...
	return client.get(infoURL)
		.then(response => {
			...
			case 'viewable':
				return info;
			case 'error':
				throw errors.buildResponseError(response, 'Representation had error status');
			case 'none':
			case 'pending':
 ...
 * @param {Boolean} [options.fullInfo] - Set to true to return representation info where all states resolve to success. Default is false
	...
	return this.client.get(apiPath, params)
		.then(response => {
			...
			case httpStatusCodes.OK:
				...
				if (options && options.fullInfo) {

					var data = response.body.representations.entries;
					var promiseArray = data.map(entry => {
						if (entry.status.state === 'success' || entry.status.state === 'viewable') {
		...
		.then(reps => {
			...
			case 'pending':
				return pollRepresentationInfo(this.client, repInfo.info.url)
					.then(info => info.content.url_template );
			default:
