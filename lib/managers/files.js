...
var BASE_PATH = '/files',
	...
	UPLOAD_SESSION_SUBRESOURCE = 'upload-session';
...
/**
 * Generate a random 8-digit hexadecimal string for use as a part ID
 * @returns {string} The hexadecimal string
 * @private
 */
function generatePartID() {

	var hex = Math.floor(Math.random() * (Math.pow(2, 32))).toString(16);
	return Array(8 - hex.length).fill('0').join('') + hex;
}
...
/**
 * Creates an upload session to create a new file in chunks.  This will first
 * verify that the file can be created and then open a session for uploading
 * pieces of the file.
 *
 * API Endpoint: '/files/upload-session'
 * Method: POST
 *
...
Files.prototype.createUploadSession = function(folderID, size, name, callback) {
	...
	var apiURL = this.client._uploadBaseURL + urlPath(BASE_PATH, UPLOAD_SESSION_SUBRESOURCE),
		params = {
			// @TODO: When API supports passing params via body, switch to body
			qs: {
				folder_id: folderID,
				file_size: size,
				file_name: name
	...
	this.client.post(apiURL, params, this.client.defaultResponseHandler(callback));
};

/**
 * Creates an upload session to create a new version of a file in chunks.  This
 * will first verify that the version can be created and then open a session for
 * uploading pieces of the file.
 *
 * API Endpoint: '/files/:fileID/upload-session'
 * Method: POST
 *
 * @param {string} fileID - The ID of the file to upload a new version of
 * @param {int} size - The size of the file that will be uploaded
 ...
 *
 * API Endpoint: '/files/upload-session/:sessionID'
 * Method: PUT
 *
 ...
 * @param {?Object} options - Optional parameters, can be left null in most cases
 * @param {string} [options.partID] - 8-character hexadecimal string uniquely denoting this part upload request
 * @param {Function} callback - Passed the part definition if successful
 * @returns {void}
 */
Files.prototype.uploadPart = function(sessionID, part, offset, totalSize, options, callback) {
	...
	this.client.put(apiURL, params, function(err, response) {
		...
		// @TODO: When API passes back chunk object, return to user directly
		callback(null, {
			part_id: partID,
			offset: offset,
			size: part.length
 ...
 *
 * API Endpoint: '/files/upload-sessions/:sessionID/commit'
 * Method: POST
 *
 ...
 * @param {UploadPart[]} parts - The list of uploaded chunks to be committed
 ...
 * @returns {void}
 */
Files.prototype.commitUploadSession = function(sessionID, fileHash, parts, options, callback) {
	...
	var apiURL = this.client._uploadBaseURL + urlPath(BASE_PATH, UPLOAD_SESSION_SUBRESOURCE, sessionID, 'commit'),
		params = {
			...
			body: {
				// @TODO: if parts was not passed in, get it from the API
				parts,
				attributes: options
			}
		};
	...
	var self = this;
	this.client.post(apiURL, params, function(err, response) {
		...
		if (response.statusCode === 202) {
			...
			setTimeout(function() {
				self.commitUploadSession(sessionID, fileHash, parts, options, callback);
			}, retryInterval * 1000);
			return;
		}
 ...
 *
 * API Endpoint: '/files/upload-sessions/:sessionID'
 * Method: DELETE
 *
 * @param {string} sessionID - The ID of the upload session to commit
...
Files.prototype.abortUploadSession = function(sessionID, callback) {
	...
	this.client.del(apiURL, null, this.client.defaultResponseHandler(callback));
};

/**
 * Get a list of all parts that have been uploaded to an upload session
 *
 * API Endpoint: '/files/upload-session/:sessionID/parts'
 * Method: GET
 *
 * @param {string} sessionID - The ID of the session to get a list of parts from
 * @param {?Object} options - Optional parameters, can be left null
 * @param {string} [options.marker] - Paging marker for the list of parts
...
Files.prototype.getUploadSessionParts = function(sessionID, options, callback) {
	...
	this.client.get(apiURL, params, this.client.defaultResponseHandler(callback));
};
 ...
 * finished committing
 *
 * API Endpoint: '/files/upload-sessions/:sessionID/status'
 * Method: GET
 *
 ...
 * @returns {void}
 */
Files.prototype.getUploadSessionStatus = function(sessionID, callback) {

	// @TODO: When GET upload-session is available, implement that instead

	var apiURL = this.client._uploadBaseURL + urlPath(BASE_PATH, UPLOAD_SESSION_SUBRESOURCE, sessionID, 'status');

	this.client.get(apiURL, null, this.client.defaultResponseHandler(callback));
};

//@TODO: add methods to use chunked upload manager
