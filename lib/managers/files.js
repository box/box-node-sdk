...
Files.prototype.deletePermanently = function(fileID, callback) {
 ...
 /**
  * Enum of valid actions for locking and unlocking files
  *
  * @enum {LockType}
  */
Files.prototype.lockType = {
	LOCK: 'lock',
	UNLOCK: 'unlock'
};

/**
* Locks and unlocks a file.
*
* API Endpoint: '/files/:fileID'
* Method: PUT
*
* @param {string} fileID - The ID of the file to lock or unlock
* @param {LockType} type - Type of the action. Can be lock or unlock
* @param {?string} [expiresAt] - The time the lock expires
* @param {boolean} [isDownloadPrevented] - Whether or not the file can be downloaded while locked
* @param {Function} callback - Called with nothing if successful, error otherwise
* @returns {void}
*/
Files.prototype.lockUnlock = function(fileID, type, expiresAt, isDownloadPrevented, callback) {
	var apiPath = urlPath(BASE_PATH, fileID),
		params = {
			body: {
				lock: {
	...
	if (expiresAt) {
		params.body.lock.expires_at = expiresAt;
	}

	if (isDownloadPrevented !== null && isDownloadPrevented !== undefined) {
		params.body.lock.is_download_prevented = isDownloadPrevented;
	}
