...
Files.prototype.commitUploadSession = function(sessionID, fileHash, options, callback) {
	...
	var self = this;
	async.waterfall([
		function getParts(waterfallCallback) {
			...
			async.doWhilst(
				function fetchParts(untilCallback) {
					...
					if (nextMarker) {
						pagingOptions.marker = nextMarker;
					}

					self.getUploadSessionParts(sessionID, pagingOptions, function(err, data) {
				...
				() => typeof nextMarker === 'string' && nextMarker.length > 0,
				function finalizeFetchParts(err) {
		...
		function commitUpload(waterfallCallback) {

			params.body.parts = parts;
			self.client.post(apiURL, params, waterfallCallback);
		}
		...
		if (response.statusCode === 202) {
			...
			setTimeout(function() {
				...
				self.commitUploadSession(sessionID, fileHash, options, callback);
			}, retryInterval * 1000);
			return;
		}
