 ...
 * @typedef {FileRepresentationType} Different representations we can request from reps endpoint
 */
 ...
 *
 * @param {string} fileID - Box ID of the file being requested
 * @param {string} representationType - The x-rep-hints value the application should create a
 *    representation for. This value can either come from FileRepresentationType enum or manually created
 * @param {Object} [options] - Additional options. Can be left null in most cases.
 * @param {string} [options.set_content_disposition_type] - Set to either 'inline' for the file to be opened in a browser tab or 'attachment'
 * for the representation to be downloaded from the browser. If not specified the default behavior is to allow the user-agent of the client
 * to determine how it should handle the returned representation object. To ensure consistency, we recommend supplying this parameter
 ...
 * @param {Function} [callback] - A representation object containing the url_template and asset_url of the representation
...
Files.prototype.getRepresentationInfo = function(fileID, representationType, options, callback) {
	var assetPath = options.asset_path || '';
	delete options.asset_path;
	...
	return this.client.get(apiPath, params)
		.then(response => {
			...
			case httpStatusCodes.OK:
				response.body.representations.entries.forEach(function(representation) {
					let template_url = representation.content.url_template;
					representation.content.asset_url = template_url.replace('{+asset_path}', assetPath);
				});
