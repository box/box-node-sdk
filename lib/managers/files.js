...
/**
 * Enum of valid x-rep- hint values for generating representation
 *
 * @readonly
...
Files.prototype.representation = {
	...
	THUMBNAIL: {
		JPG : {
			SMALL: '[jpg?dimensions=32x32]',
			MEDIUM: '[jpg?dimensions=1024x1024]',
			LARGE: '[jpg?dimensions=2048x2048]'
		},
		PNG: {
			MEDIUM: '[png?dimensions=1024x1024]',
			LARGE: '[png?dimensions=2048x2048]'
		}
	},
	MULTI_PAGE: {
		MEDIUM:'[png?dimensions=1024x1024]',
...
/**
 * Requests links to all representations generated for a specific Box file
 *
 * API Endpoint: '/files/:fileID?fields=representations'
 * Method : GET
 *
 * @param {string} fileID - Box ID of the file being requested
 * @param {FileRepresentationType} representationType - The x-rep-hints value the application should create a
 *    representation for. This value should be access from enum
 * @param {Object} [options] - Additional options. Can be left null in most cases.
 * @param {string} [options.set_content_disposition_type] - Set to either 'inline' for the file to be downloaded from the
 *    browser or 'attachment' which opens the file in another tab
 * @param {string} [options.set_content_disposition_filename] - If defined this will name the downloaded representation file, if not the name will
 *    be the original Box file name
 * @param {string} [options.asset_path] - This optional field is for requesting a multi-page representation. This value should be in format of
			<page_number>.<png>. 4-page long document will be named: 1.png, 2.png, etc...
 * @param {Function} [callback] - Passed  a url link to download the representation that was generated
 * @returns {Promise<Object>} A promise resolving to the url links to of all file representations generated
 *    for Box files
*/
Files.prototype.getRepresentationInfo = function(fileID, representationType, options, callback) {
	...
	Object.assign(params.headers, options);
	var apiPath = urlPath(BASE_PATH, fileID);
	...
	return this.client.get(apiPath, params)
		.then(response => {
			...
			// 200 - A Boxfile representation image received
			// return the representation file
			case httpStatusCodes.OK:
				if(response.body.representations.entries[0].metadata){
					templateUrl = response.body.representations.entries[0].content.url_template.replace("{+asset_path}", options.asset_path);
				}else{
					templateUrl = response.body.representations.entries[0].content.url_template.replace("{+asset_path}", "");
				}
				return {
					statusCode: response.statusCode,
					representationUrl: templateUrl
				};
