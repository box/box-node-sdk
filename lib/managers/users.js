 ...
 * @param {boolean} [notify = false] - Determines if the destination user should receive email notification of the transfer
 * @param {boolean} [force = false] - Whether or not the user should be deleted even if this user still own files
 * @param {Function} callback - Empty response body passed if successful, error otherwise
 * @returns {void}
 */
Users.prototype.delete = function(id, notify, force, callback) {
	var apiPath = urlPath(BASE_PATH, id),
		params = {
			body: {}
		};
	...
	if (notify !== null && notify !== undefined) {
		params.body.notify = notify;
	}

	if (force !== null && force !== undefined) {
		params.body.force = force;
	}
