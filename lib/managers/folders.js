 ...
 *
 * @param  {type} folderID  - The ID of the folder being requested
 * @param {?Object} qs - Additional options can be passed with the request via querystring. Can be left null in most cases.
 * @param  {type} callback  - Passed the folder information if it was acquired successfully
 * @returns {void}
 */
Folders.prototype.getTrashedFolder = function(folderID, qs, callback) {
	...
	this.client.get(apiPath, params, function(err, response) {
		...
		case httpStatusCodes.OK:
			...
			// 404 - Not Found
			// Folder is not in the trash
		case httpStatusCodes.NOT_FOUND:
			callback(errors.buildResponseError(response, 'The folder is not in the trash'));
			return;
