...
/**
 * Requests a folder object at the given absolute or relative path
 *
 * API Endpoint: '/folders?path=:path'
 * Method: GET
 *
 * @param {string} path - The path to the folder requested
 * @param {Object} [options] - Optional parameters
 * @param {string} [options.parent_id] - If the path is relative, the ID of the folder it should be relative to
 * @param {string} [options.fields] - Comma-separated list of fields to display on the folder object
 * @param {Function} [callback] - Passed the folder information if call was successful
 * @returns {Promise<Object>} A promise resolving to the folder object
 */
Folders.prototype.getByPath = function(path, options, callback) {

	var params = {
		qs: Object.assign({ path }, options)
	};
	...
	return this.client.get(apiPath, params)
		.then(response => {
			...
			if (response.body.entries.length === 0) {
				throw new Error('Could not find requested file');
			}
