...
```ts
await client.skills.createBoxSkillCardsOnFile(file.id, { cards: cardsToCreate } satisfies CreateBoxSkillCardsOnFileRequestBody)
```
...
```ts
await client.skills.updateBoxSkillCardsOnFile(file.id, [{ op: "replace" as UpdateBoxSkillCardsOnFileRequestBodyOpField, path: "/cards/0", value: cardToUpdate } satisfies UpdateBoxSkillCardsOnFileRequestBody])
```
