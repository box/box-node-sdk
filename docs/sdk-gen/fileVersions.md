...
```ts
await client.fileVersions.updateFileVersionById(file.id, fileVersion.id, { requestBody: { trashedAt: createNull() } satisfies UpdateFileVersionByIdRequestBody } satisfies UpdateFileVersionByIdOptionalsInput)
```
...
```ts
await client.fileVersions.promoteFileVersion(file.id, { requestBody: { id: fileVersions.entries![0].id, type: "file_version" as PromoteFileVersionRequestBodyTypeField } satisfies PromoteFileVersionRequestBody } satisfies PromoteFileVersionOptionalsInput)
```
