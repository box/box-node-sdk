...
```ts
await client.hubItems.getHubItemsV2025R0({ hubId: createdHub.id } satisfies GetHubItemsV2025R0QueryParams)
```
...
```ts
await client.hubItems.manageHubItemsV2025R0(createdHub.id, { operations: [{ action: "add" as HubItemOperationV2025R0ActionField, item: new FolderReferenceV2025R0({ id: folder.id }) } satisfies HubItemOperationV2025R0] } satisfies HubItemsManageRequestV2025R0)
```
