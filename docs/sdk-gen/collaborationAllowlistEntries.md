...
```ts
await client.collaborationAllowlistEntries.createCollaborationWhitelistEntry({ direction: "inbound" as CreateCollaborationWhitelistEntryRequestBodyDirectionField, domain: domain } satisfies CreateCollaborationWhitelistEntryRequestBody)
```
...
```ts
await client.collaborationAllowlistEntries.getCollaborationWhitelistEntryById(newEntry.id!)
```
