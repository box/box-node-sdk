...
```ts
await client.folderMetadata.getFolderMetadataById(folder.id, "global" as GetFolderMetadataByIdScope, "properties")
```
...
```ts
await client.folderMetadata.createFolderMetadataById(folder.id, "enterprise" as CreateFolderMetadataByIdScope, templateKey, { ["name"]: "John", ["age"]: 23, ["birthDate"]: "2001-01-03T02:20:50.520Z", ["countryCode"]: "US", ["sports"]: ["basketball", "tennis"] })
```
...
```ts
await client.folderMetadata.deleteFolderMetadataById(folder.id, "enterprise" as DeleteFolderMetadataByIdScope, templateKey)
```
