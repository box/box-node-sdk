...
```ts
await client.aiStudio.createAiAgent({ name: agentName, accessState: "enabled", ask: new AiStudioAgentAsk({ accessState: "enabled", description: "desc1" }) } satisfies CreateAiAgentInput)
```
...
```ts
await client.aiStudio.updateAiAgentById(createdAgent.id, { name: agentName, accessState: "enabled", ask: new AiStudioAgentAsk({ accessState: "disabled", description: "desc2" }) } satisfies CreateAiAgentInput)
```
