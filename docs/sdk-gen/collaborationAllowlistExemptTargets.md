...
```ts
await client.collaborationAllowlistExemptTargets.createCollaborationWhitelistExemptTarget({ user: { id: user.id } satisfies CreateCollaborationWhitelistExemptTargetRequestBodyUserField } satisfies CreateCollaborationWhitelistExemptTargetRequestBody)
```
...
```ts
await client.collaborationAllowlistExemptTargets.getCollaborationWhitelistExemptTargetById(newExemptTarget.id!)
```
