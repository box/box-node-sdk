...
```ts
await client.comments.updateCommentById(newReplyComment.id!, { requestBody: { message: newMessage } satisfies UpdateCommentByIdRequestBody } satisfies UpdateCommentByIdOptionalsInput)
```
...
```ts
await client.comments.createComment({ message: message, item: { id: fileId, type: "file" as CreateCommentRequestBodyItemTypeField } satisfies CreateCommentRequestBodyItemField } satisfies CreateCommentRequestBody)
```
