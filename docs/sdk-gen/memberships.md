...
```ts
await client.memberships.createGroupMembership({ user: { id: user.id } satisfies CreateGroupMembershipRequestBodyUserField, group: { id: group.id } satisfies CreateGroupMembershipRequestBodyGroupField } satisfies CreateGroupMembershipRequestBody)
```
...
```ts
await client.memberships.updateGroupMembershipById(groupMembership.id!, { requestBody: { role: "admin" as UpdateGroupMembershipByIdRequestBodyRoleField } satisfies UpdateGroupMembershipByIdRequestBody } satisfies UpdateGroupMembershipByIdOptionalsInput)
```
