...
<!-- sample post_groups -->
```ts
await client.groups.createGroup({ name: groupName, description: groupDescription } satisfies CreateGroupRequestBody)
```
...
```ts
await client.groups.getGroupById(group.id, { queryParams: { fields: ["id", "name", "description", "group_type"] } satisfies GetGroupByIdQueryParams } satisfies GetGroupByIdOptionalsInput)
```
...
```ts
await client.groups.updateGroupById(group.id, { requestBody: { name: updatedGroupName } satisfies UpdateGroupByIdRequestBody } satisfies UpdateGroupByIdOptionalsInput)
```
