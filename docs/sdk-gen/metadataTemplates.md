...
```ts
await client.metadataTemplates.getMetadataTemplatesByInstanceId({ metadataInstanceId: createdMetadataInstance.id! } satisfies GetMetadataTemplatesByInstanceIdQueryParams)
```
...
```ts
await client.metadataTemplates.getMetadataTemplate("enterprise" as GetMetadataTemplateScope, template.templateKey!)
```
...
```ts
await client.metadataTemplates.updateMetadataTemplate("enterprise" as UpdateMetadataTemplateScope, templateKey, [{ op: "addField" as UpdateMetadataTemplateRequestBodyOpField, fieldKey: "newfieldname", data: { ["type"]: "string", ["displayName"]: "newFieldName" } } satisfies UpdateMetadataTemplateRequestBody])
```
...
```ts
await client.metadataTemplates.deleteMetadataTemplate("enterprise" as DeleteMetadataTemplateScope, template.templateKey!)
```
