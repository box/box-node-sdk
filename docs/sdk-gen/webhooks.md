...
```ts
await client.webhooks.createWebhook({ target: { id: folder.id, type: "folder" as CreateWebhookRequestBodyTargetTypeField } satisfies CreateWebhookRequestBodyTargetField, address: "https://example.com/new-webhook", triggers: ["FILE.UPLOADED" as CreateWebhookRequestBodyTriggersField] } satisfies CreateWebhookRequestBody)
```
...
```ts
await client.webhooks.updateWebhookById(webhook.id!, { requestBody: { address: "https://example.com/updated-webhook" } satisfies UpdateWebhookByIdRequestBody } satisfies UpdateWebhookByIdOptionalsInput)
```
...
```ts
await WebhooksManager.validateMessage(body, headersWithCorrectDatetime, primaryKey, { secondaryKey: secondaryKey } satisfies ValidateMessageOptionalsInput)
```
