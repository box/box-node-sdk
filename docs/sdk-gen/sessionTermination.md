...
```ts
await client.sessionTermination.terminateUsersSessions({ userIds: [getEnvVar("USER_ID")], userLogins: [user.login!] } satisfies TerminateUsersSessionsRequestBody)
```
...
```ts
await client.sessionTermination.terminateGroupsSessions({ groupIds: [group.id] } satisfies TerminateGroupsSessionsRequestBody)
```
