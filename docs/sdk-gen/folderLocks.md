...
```ts
await client.folderLocks.getFolderLocks({ folderId: folder.id } satisfies GetFolderLocksQueryParams)
```
...
```ts
await client.folderLocks.createFolderLock({ folder: { id: folder.id, type: "folder" } satisfies CreateFolderLockRequestBodyFolderField, lockedOperations: { move: true, delete: true } satisfies CreateFolderLockRequestBodyLockedOperationsField } satisfies CreateFolderLockRequestBody)
```
