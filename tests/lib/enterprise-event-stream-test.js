...
describe('EnterpriseEventStream', () => {
  ...
  describe('constructor', () => {
    ...
    it('should set all of the options correctly', () => {
      ...
      const enterpriseEventStream2 = new EnterpriseEventStream(
        boxClientFake,
        options
      );
      assert.propertyVal(
        enterpriseEventStream2,
        '_streamPosition',
        TEST_STREAM_POSITION
      );
      assert.propertyVal(
        enterpriseEventStream2._options,
        'startDate',
        TEST_DATE
      );
      assert.propertyVal(
    ...
    it('should set start date to now when stream position and start date are not present', () => {
      ...
      assert.equal(
        Date.parse(enterpriseEventStream2._options.startDate),
        sandbox.clock.now
      );
    });
  });
  describe('getStreamPosition()', () => {
    it('should return the current stream position when called', () => {
      assert.equal(
        enterpriseEventStream.getStreamPosition(),
        TEST_STREAM_POSITION
      );
    });
  });
  describe('setStreamState()', () => {
    it('should set the current stream state', () => {
      ...
      assert.propertyVal(
        enterpriseEventStream._options,
        'startDate',
        TEST_DATE
      );
      ...
      it('should make API call to get events with the correct options when called', () => {
        ...
        sandbox
          ...
          .withArgs(
            sinon.match(
              (value) =>
                value.stream_type === 'admin_logs_streaming' &&
  ...
  describe('_read()', () => {
    it('should push the events into the stream when fetchEvents() return events', () => {
      sandbox
        .stub(enterpriseEventStream, 'fetchEvents')
        .yields(null, [TEST_EVENT_1, TEST_EVENT_2]);
    ...
    it('should ony call _read() once after each call to fetchEvents()', (done) => {
      ...
      sandbox
        .stub(enterpriseEventStream, 'fetchEvents')
        .onFirstCall()
        .yieldsAsync(null, [TEST_EVENT_1, TEST_EVENT_2, TEST_EVENT_3])
        .onSecondCall()
        .yieldsAsync(null, [])
        .onThirdCall()
        .throws();
