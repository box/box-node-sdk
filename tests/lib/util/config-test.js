...
describe('Config', function () {
  describe('constructor', function () {
    const TEST_PRIVATE_KEY =
        'wieuth;jwnfp9qun3r;ifjaodfuihglwidjnfgiauhrtp9qbdlkfjbaeirug',
      TEST_KEY_ID = '7893y45',
    ...
    it('should correctly freeze config objects with Streams in them', function () {
      ...
      config.request.body.on('data', () => {
        /* empty */
      }); // this would throw if the stream was frozen
    ...
    leche.withData(
      {
        ...
        'missing private key': [
          { keyID: TEST_KEY_ID, passphrase: TEST_PASSPHRASE },
        ],
        'missing passphrase': [
          { keyID: TEST_KEY_ID, privateKey: TEST_PRIVATE_KEY },
        ],
        'unsupported algorithm': [
          Object.assign({ algorithm: 'HS4096' }, TEST_APP_AUTH_PARAMS),
        ],
        'float expiration time': [
          Object.assign({ expirationTime: 5.5 }, TEST_APP_AUTH_PARAMS),
        ],
        'zero expiration time': [
          Object.assign({ expirationTime: 0 }, TEST_APP_AUTH_PARAMS),
        ],
        'too-long expiration time': [
          Object.assign({ expirationTime: 61 }, TEST_APP_AUTH_PARAMS),
        ],
      },
  ...
  describe('extend()', function () {
    ...
    it('should retain properties set by a previous extension', function () {
      var doubleExtendedConfig = config
        .extend({
          clientID: 'newID',
        })
        .extend({
