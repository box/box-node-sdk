...
describe('EventStream', function () {
  ...
  describe('getLongPollInfo()', function () {
    it('should make API call and set long poll info when called', function () {
      ...
      sandbox
        .mock(boxClientFake.events)
        .expects('getLongPollInfo')
        .returns(Promise.resolve(longPollInfo));

      return eventStream.getLongPollInfo().then(() => {
    ...
    it('should reset the number of long poll retries when API call is successful', function () {
      ...
      sandbox.stub(eventStream, 'doLongPoll');
      sandbox
        .stub(boxClientFake.events, 'getLongPollInfo')
        .returns(Promise.resolve(longPollInfo));
    ...
    describe('when stream is destroyed', function () {
      ...
      it('should do nothing and resolve `false`', function () {
        sandbox
          .mock(eventStream._client.events)
          .expects('getLongPollInfo')
          .never();

        return eventStream.getLongPollInfo().then((result) => {
  ...
  describe('doLongPoll()', function () {
    ...
    it('should reset long poll process immediately when API call returns reconnect message', function () {
      sandbox.stub(boxClientFake, 'wrapWithDefaultHandler').returnsArg(0);
      sandbox.stub(boxClientFake, 'get').returns(
        Promise.resolve({
          message: 'reconnect',
        })
      );
    ...
    describe('when stream is destroyed', function () {
      describe('before client fetch', function () {
        ...
        it('should do nothing and resolve `false`', function () {
          sandbox.mock(eventStream).expects('getLongPollInfo').never();
          sandbox.mock(boxClientFake).expects('wrapWithDefaultHandler').never();

          return eventStream.doLongPoll().then((result) => {
            assert.strictEqual(result, false);
          });
        });
      });
  ...
  describe('fetchEvents()', function () {
    ...
    it('should re-poll when API call does not return any events', function () {
      sandbox.stub(boxClientFake.events, 'get').returns(
        Promise.resolve({
          chunk_size: 0,
    ...
    it('should delay successive calls to be rate limited when called', function () {
      ...
      var p1 = eventStream.fetchEvents().then(() => {
        called.first = true;
      ...
      var p2 = eventStream.fetchEvents().then(() => {
        called.second = true;
    ...
    describe('when stream is destroyed before fetch', function () {
      ...
      it('should do nothing and resolve `false`', function () {
        sandbox.mock(eventStream._rateLimiter).expects('then').never();

        return eventStream.fetchEvents().then((result) => {
          assert.strictEqual(result, false);
        });
      });
    });
