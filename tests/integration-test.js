...
describe('Box Node SDK', function () {
  ...
  it('should use exponential backoff with randomization when API calls fail', function () {
    ...
    var start = Date.now();
    return client.users.get('me').then(() => {
      var end = Date.now();
  ...
  it('should get anonymous tokens and make API call when anonymous client manager is used', function (done) {
    ...
    apiMock
      ...
      .matchHeader('BoxApi', function (boxHeader) {
        assert.equal(
          boxHeader,
          `shared_link=${encodeURIComponent(sharedLink)}`
        );
        return true;
      })
      .reply(200, {
  ...
  it('should correctly store tokens and make API call when app auth client manager is called', function (done) {
    ...
    var privateKey = fs.readFileSync(
      path.resolve(__dirname, 'fixtures/appusers_private_key.pem')
    );
  ...
  it('should correctly exchange tokens when client token exchange is called', function (done) {
    ...
    var exchangedTokenInfo = {
      access_token:
        '1!938y4fncqwe8rqwoieo-05oe8ugisuedb8745y45756dfgbdfgu-wrtwergwre5yfdgber623rgfadsftw45yehr-dfg',
      expires_in: 3987,
      token_type: 'bearer',
      restricted_to: [
        {
  ...
  it('should fail request and call session handler when response is token expiration error', function () {
    ...
    var fakeTokenStore = leche.create(['read', 'write', 'clear']);
    ...
    sandbox.mock(fakeTokenStore).expects('clear').yieldsAsync();
  ...
  it('should return file stream when file read stream is requested', function (done) {
    ...
    apiMock
      ...
      .reply(function () {
        return [302, '', { location: fileDownloadRoot + fileDownloadPath }];
      });

    dlMock.get(fileDownloadPath).reply(200, function () {
      return fileStream;
    });
  ...
  it('should upload file in chunks when chunked upload is requested and run', function (done) {
    ...
    uploadMock
      .post('/api/2.0/files/upload_sessions', function (body) {
      ...
      .reply(201, {
        ...
        session_endpoints: {
          list_parts:
            'https://upload.box.com/api/2.0/files/upload-session/07C4B58DF2D79928A787CCB99A5FF37E/parts',
          commit:
            'https://upload.box.com/api/2.0/files/upload-session/07C4B58DF2D79928A787CCB99A5FF37E/commit',
          log_event:
            'https://upload.box.com/api/2.0/files/upload-session/07C4B58DF2D79928A787CCB99A5FF37E/log',
          upload_part:
            'https://upload.box.com/api/2.0/files/upload-session/07C4B58DF2D79928A787CCB99A5FF37E',
          status:
            'https://upload.box.com/api/2.0/files/upload-session/07C4B58DF2D79928A787CCB99A5FF37E',
          abort:
            'https://upload.box.com/api/2.0/files/upload-session/07C4B58DF2D79928A787CCB99A5FF37E',
      ...
      .put(
        `/api/2.0/files/upload_sessions/${uploadSessionID}`,
        function (body) {
      ...
      .matchHeader('Digest', function (digestHeader) {
        var expected = crypto
          .createHash('sha1')
          .update(fileContents.slice(0, 210))
          .digest('base64');
      ...
      .matchHeader('Digest', function (digestHeader) {
        assert.equal(
          ...
          `SHA=${crypto
            .createHash('sha1')
            .update(fileContents.slice(210, 420))
            .digest('base64')}`
        );
        return true;
      })
      ...
      .matchHeader('Digest', function (digestHeader) {
        assert.equal(
          ...
          `SHA=${crypto
            .createHash('sha1')
            .update(fileContents.slice(420, 630))
            .digest('base64')}`
        );
        return true;
      })
      ...
      .matchHeader('Digest', function (digestHeader) {
        assert.equal(
          ...
          `SHA=${crypto
            .createHash('sha1')
            .update(fileContents.slice(630, 840))
            .digest('base64')}`
        );
        return true;
      })
      ...
      .post(
        `/api/2.0/files/upload_sessions/${uploadSessionID}/commit`,
        function (body) {
  ...
  it('should upload with file attributes and return promise when chunked upload is executed', function () {
    ...
    return client.files
      .getChunkedUploader(folderID, fileSize, fileName, fileStream, {
        fileAttributes,
      })
  ...
  it('should correctly upload stream from request module when using chunked uploader', function (done) {
    ...
    request
      .get('https://www.example.com/test-file.pdf')
      .on('response', (responseStream) => {
        client.files
          .getChunkedUploader('1234', fileSize, fileName, responseStream)
          .then((uploader) => {
  ...
  it('should correctly reconfigure SDK instance', function () {
    ...
    return client.folders.get(folderID).then((folder) => {
