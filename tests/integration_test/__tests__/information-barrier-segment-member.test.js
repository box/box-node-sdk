...
test('test information barrier segment member', async () => {
  let barrierSegmentMember =
    await context.client.shieldInformationBarrierSegmentMembers.create({
      shield_information_barrier_segment: context.barrierSegment,
      user: context.user,
    });
  expect(barrierSegmentMember.id).toBeDefined();
  expect(barrierSegmentMember.shield_information_barrier_segment.id).toBe(
    context.barrierSegment.id
  );
  expect(barrierSegmentMember.user.id).toBe(context.user.id);

  barrierSegmentMember =
    await context.client.shieldInformationBarrierSegmentMembers.getById({
  ...
  const barrierSegmentMembers =
    await context.client.shieldInformationBarrierSegmentMembers.getAll({
