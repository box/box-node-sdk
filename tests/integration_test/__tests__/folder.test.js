...
const {
  ...
  createLocalFile,
  removeLocalFile,
} = require('../objects/box-local-file');
...
} = require('../objects/box-test-metadata-template');
const {
  createBoxTestMetadataCascadePolicies,
} = require('../objects/box-test-metadata-cascade-policies');
const {
...
test('test preflight check', async () => {
  let fileSize = 123;
  let result = await context.client.files.preflightUploadFile(
    context.folder.id,
    {
      name: 'test.txt',
      size: fileSize,
...
test('test manual chunked upload', async () => {
  ...
  let uploadSession = await context.client.files.createUploadSession(
    context.folder.id,
    stats.size,
    path.basename(file)
  );
  for (let i = 0; i < uploadSession.total_parts; i++) {
    let chunkSize = Math.min(
      uploadSession.part_size,
      stats.size - i * uploadSession.part_size
    );
    let buffer = Buffer.alloc(chunkSize);
    fs.readSync(fd, buffer, 0, chunkSize, i * uploadSession.part_size);
    /* eslint-disable no-await-in-loop */
    let result = await context.client.files.uploadPart(
      uploadSession.id,
      buffer,
      i * uploadSession.part_size,
      stats.size
    );
  ...
  let filehash = crypto
    ...
    .digest('base64');
  let uploadedFile = await context.client.files.commitUploadSession(
    uploadSession.id,
    filehash,
    {
...
test('test auto chunked upload', async () => {
  ...
  let uploader = await context.client.files.getChunkedUploader(
    context.folder.id,
    stats.size,
    path.basename(file),
    stream
...
test('test get items', async () => {
  let testFile = await createBoxTestFolder(
    context.client,
    null,
    context.folder.id
  );
...
test('test upload stream to folder', async () => {
  ...
  try {
    let stream = fs.createReadStream(file);
    let uploadedFile = await context.client.files.uploadFile(
      context.folder.id,
      path.basename(file),
      stream
    );
...
test('test create subfolder', async () => {
  let subfolder = await createBoxTestFolder(
    context.client,
    null,
    context.folder.id
  );
...
test('test add collaborator', async () => {
  ...
  try {
    let result = await context.client.collaborations.createWithUserID(
      testUser.id,
      subfolder.id,
      context.client.collaborationRoles.EDITOR
    );
    expect(result).toBeDefined();
    let collaborations = await context.client.folders.getCollaborations(
      subfolder.id
    );
...
test('test invite collaborator using email', async () => {
  ...
  try {
    let result = await context.client.collaborations.createWithUserEmail(
      testUser.login,
      subfolder.id,
      context.client.collaborationRoles.EDITOR
    );
...
test('test get shared link', async () => {
  ...
  try {
    await context.client.collaborations.createWithUserID(
      testUser.id,
      subfolder.id,
      context.client.collaborationRoles.EDITOR
    );
    ...
    let sharedLink = await testClient.folders.get(subfolder.id, {
      fields: 'shared_link',
    });
...
test('test create weblink', async () => {
  let weblink = await context.client.weblinks.create(
    'https://www.box.com',
    context.folder.id,
    {
      name: 'Box Website',
...
test('test cascade and get metadata cascade policies', async () => {
  ...
  let cascadePolicy = await createBoxTestMetadataCascadePolicies(
    context.client,
    subfolder.id,
    metadataTemplate.templateKey
  );
  try {
    let policies = await context.client.metadata.getCascadePolicies(
      subfolder.id
    );
...
test('test add and remove collection', async () => {
  ...
  try {
    ...
    await context.client.folders.removeFromCollection(
      subfolder.id,
      collection.id
    );
...
test('test move folder', async () => {
  ...
  let newFolder = await createBoxTestFolder(
    context.client,
    null,
    context.folder.id
  );
...
test('test folder metadata', async () => {
  ...
  try {
    ...
    await context.client.folders.setMetadata(
      subfolder.id,
      context.client.metadata.scopes.ENTERPRISE,
      metadataTemplate.templateKey,
      {}
    );
    metadatas = await context.client.folders.getAllMetadata(subfolder.id);
    expect(metadatas.entries.length).toBe(count + 1);
    await context.client.folders.deleteMetadata(
      subfolder.id,
      context.client.metadata.scopes.ENTERPRISE,
      metadataTemplate.templateKey
    );
...
test('test trash folder', async () => {
  ...
  try {
    await context.client.folders.delete(subfolder.id);
    let trashedFolders = await context.client.folders.getTrashedFolder(
      subfolder.id
    );
