...
test('test get file with custom dispostion time', async () => {
  let testRetentionPolicy = await createBoxTestRetentionPolicy(
    context.appClient
  );
  ...
  try {
    retentionPolicyAssignment =
      await context.appClient.retentionPolicies.assign(
        testRetentionPolicy.id,
        testFolder.type,
        testFolder.id
      );
    testFile = await createBoxTestFile(
      context.appClient,
      path.join(__dirname, '../resources/blank.pdf'),
      'testfile.pdf',
      testFolder.id
    ...
    let file = await context.appClient.files.get(testFile.id, {
      fields: 'created_at,disposition_at',
    });
    expect(file.id).toBe(testFile.id);
    expect(file.disposition_at).not.toBeNull();
    let disposeAt = new Date(file.disposition_at);
    let createdAt = new Date(file.created_at);
    expect(createdAt.addDays(1).toLocaleDateString()).toBe(
      disposeAt.toLocaleDateString()
    );

    let newDisposeAt = new Date(createdAt.addDays(2));
    await context.appClient.files.update(testFile.id, {
      disposition_at: newDisposeAt.toISOString().replace('.000Z', 'Z'),
      fields: 'disposition_at',
    });
    file = await context.appClient.files.get(testFile.id, {
      fields: 'disposition_at',
    });
    disposeAt = new Date(file.disposition_at);
    expect(newDisposeAt.toLocaleDateString()).toBe(
      disposeAt.toLocaleDateString()
    );
  } finally {
    if (retentionPolicyAssignment) {
      await context.appClient.retentionPolicies.deleteAssignment(
