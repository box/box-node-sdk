...
test('test information barrier segment', async () => {
  ...
  barrierSegment =
    await context.client.shieldInformationBarrierSegments.getById({
      shield_information_barrier_segment_id: barrierSegment.id,
    });
  expect(barrierSegment.id).toBeDefined();
  ...
  const barrierSegments =
    await context.client.shieldInformationBarrierSegments.getAll({
      shield_information_barrier_id: context.barrier.id,
    });
  expect(barrierSegments.entries).toBeDefined();
  expect(
    barrierSegments.entries.find((item) => item.id === barrierSegment.id)
  ).toBeUndefined();
});
