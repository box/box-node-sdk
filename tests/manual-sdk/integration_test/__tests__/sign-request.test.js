...
test('test sign request', async () => {
  let file1 = await createBoxTestFile(
    context.client,
    path.join(__dirname, '../resources/blank.pdf'),
    'blank_sign_1.pdf',
    context.folder.id
  );
  let file2 = await createBoxTestFile(
    context.client,
    path.join(__dirname, '../resources/blank.pdf'),
    'blank_sign_2.pdf',
    context.folder.id
  ...
  try {
      ...
      signers: [
        {
          ...
          declined_redirect_url:
            'https://www.box.com/declined_redirect_url_singer_1',
        },
      ],
      source_files: [
        {
    ...
    expect(sr.declined_redirect_url).toBe(
      'https://www.box.com/declined_redirect_url'
    );
    ...
    for (let signer of sr.signers) {
      if (signer.role === 'signer') {
        ...
        expect(signer.redirect_url).toBe(
          'https://www.box.com/redirect_url_signer_1'
        );
        expect(signer.declined_redirect_url).toBe(
          'https://www.box.com/declined_redirect_url_singer_1'
        );
...
test('test sign request with signer group', async () => {
  let file = await createBoxTestFile(
    context.client,
    path.join(__dirname, '../resources/blank.pdf'),
    'blank_sign_1.pdf',
    context.folder.id
