...
test('test metadata search', async () => {
  ...
  const metadataTemplate = await context.appClient.metadata.createTemplate(
    templateKey,
    fields,
    {
      scope: 'enterprise',
  ...
  try {
    ...
    try {
      const metadata = await context.appClient.files.addMetadata(
        file.id,
        'enterprise',
        templateKey,
        { testFloatValue: 150 }
      ...
      const searchResults = await context.appClient.metadata.query(
        searchForm,
        ancestorFolderId,
        queryOptions
      );
    ...
    await context.appClient.metadata.deleteTemplate(
      'enterprise',
