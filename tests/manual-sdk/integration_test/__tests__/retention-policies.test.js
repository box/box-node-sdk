...
test('test retention policy', async () => {
  ...
  expect(retentionPolicy.description).toBe(
  ...
  retentionPolicy = await context.appClient.retentionPolicies.get(
    retentionPolicy.id
  );
  expect(retentionPolicy.id).toBeDefined();
  expect(retentionPolicy.policy_name).toBe(policyName);
  ...
  expect(retentionPolicy.description).toBe(
    'The modified text description of the retention policy'
  );
}, 180000);
...
test('test retention policy assignment for folder', async () => {
  ...
  try {
    ...
    expect(assignment.retention_policy.policy_id).toBe(
      testRetentionPolicy.policy_id
    );
    expect(assignment.assigned_to.type).toBe('folder');
    expect(assignment.assigned_to.id).toBe(testFolder.id);

    assignment = await context.appClient.retentionPolicies.getAssignment(
      assignment.id
    );
    expect(assignment.retention_policy.policy_id).toBe(
      testRetentionPolicy.policy_id
