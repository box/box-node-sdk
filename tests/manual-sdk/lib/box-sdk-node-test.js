...
describe('box-node-sdk', function () {
  ...
  describe('constructor', function () {
    it('should set config for all passed in params when called', function () {
      ...
      assert.ok(
        TokenManagerConstructorStub.calledWithNew(),
        'Should construct new TokenManager'
      );
      assert.ok(
        TokenManagerConstructorStub.calledWithMatch(TEST_CONFIG),
        'TokenManager should be passed config'
      );
    ...
    it('should create an anonymous session with config when called', function () {
      ...
      assert.ok(
        CCGAPISession.calledWithNew(),
        'Should construct new anonymous session'
      );
      assert.ok(
        CCGAPISession.calledWithMatch(TEST_CONFIG),
        'Anonymous session should be passed config'
      );
      assert.ok(sdk, 'SDK should be constructed');
    });
    ...
    it('should create an API Request Manager with config when called', function () {
      ...
      assert.ok(
        APIRequestManagerConstructorStub.calledWithNew(),
        'Should construct new APIRequestManager'
      );
      assert.ok(
        APIRequestManagerConstructorStub.calledWithMatch(TEST_CONFIG),
        'APIRequestManager should be passed config'
      );
      assert.instanceOf(
        APIRequestManagerConstructorStub.getCall(0).args[1],
        EventEmitter,
        'APIRequestManager should be passed event bus'
      );
      assert.ok(sdk, 'SDK should be constructed');
    });
    ...
    it('should set default apiRootURL when one is given', function () {
      ...
      assert.ok(
        TokenManagerConstructorStub.calledWithMatch(expectedParams),
        'TokenManager should be passed correct config values'
      );
      assert.ok(sdk, 'SDK should be constructed');
    });
  });
  ...
  describe('getPreconfiguredInstance()', function () {
    it('should set config for all passed in params when called', function () {
      ...
      assert.ok(
        TokenManagerConstructorStub.calledWithMatch(TEST_APP_SETTINGS_CONFIG),
        'TokenManager should be passed config'
      );
      assert.ok(sdk, 'SDK should be constructed');
    });
    ...
    it('should create an anonymous session with config when called', function () {
      ...
      assert.ok(
        CCGAPISession.calledWithMatch(TEST_APP_SETTINGS_CONFIG),
        'Anonymous session should be passed config'
      );
      assert.ok(sdk, 'SDK should be constructed');
    });
    ...
    it('should create an API Request Manager with config when called', function () {
      ...
      assert.ok(
        APIRequestManagerConstructorStub.calledWithMatch(
          TEST_APP_SETTINGS_CONFIG
        ),
        'APIRequestManager should be passed config'
      );
  ...
  describe('getBasicClient()', function () {
    ...
    it('should return an instance of a Basic Client when called', function () {
      ...
      assert.ok(
        basicClient instanceof BasicClient,
        'Returned instance of Basic Client'
      );
    });
  ...
  describe('getPersistentClient()', function () {
    ...
    it('should return an instance of a Persistent Client when called', function () {
      ...
      assert.ok(
        persistentClient instanceof BasicClient,
        'Returned instance of Peristent Client'
      );
    });
  });
  ...
  describe('getAnonymousClient()', function () {
    ...
    it('should return an instance of an Anonymous Client when called', function () {
      ...
      assert.ok(
        anonymousClient instanceof BasicClient,
        'Returned instance of Anonymous Client'
      );
    });
    ...
    it('should use the SDK anonymous session when called', function () {
      ...
      assert.ok(
        CCGAPISession.calledWithMatch(CCG_CONFIG),
        'Anonymous session should be passed config'
      );
      assert.ok(
        BasicClient.calledWithMatch(anonymousSession, CCG_CONFIG),
        'Anonymous session should be passed in'
      );
      assert.ok(
        TokenManagerConstructorStub.calledWithNew(),
        'New token manager should be created'
      );
      assert.ok(
        TokenManagerConstructorStub.calledWithMatch(
          {
            boxSubjectType: 'enterprise',
            boxSubjectId: 'enterprise-id',
          },
          requestManagerFake
        ),
  ...
  describe('getCCGClientForUser()', function () {
    ...
    it('should use the SDK anonymous session when called', function () {
      ...
      assert.ok(
        TokenManagerConstructorStub.calledWithMatch(
          { boxSubjectType: 'user', boxSubjectId: 'user-id' },
          requestManagerFake
        ),
        'New token manager should be configured for CCG'
      );
  ...
  describe('getAppAuthClient()', function () {
    ...
    it('should return a client using AppAuthSession when called', function () {
      ...
      assert.ok(BasicClient.calledWithNew(), 'New client should be created');
      assert.ok(
        BasicClient.calledWithMatch(appAuthSessionFake),
        'App auth session should be passed in'
      );
    });
    ...
    it('should use the enterprise id that is passed as a parameter and should not use the config', function () {
      ...
      assert.ok(
        AppAuthSessionConstructorStub.calledWithNew(),
        'New client should be created'
      );
      assert.ok(
        AppAuthSessionConstructorStub.calledWithMatch(
          type,
          id,
          TEST_APP_SETTINGS_CONFIG,
          tokenManagerFake
        ),
        'Enterprise id should match the parameter value'
      );
    });
    ...
    it('should use enterprise ID from config when one is set and none is passed in', function () {
      ...
      assert.ok(
        AppAuthSessionConstructorStub.calledWithMatch(
          type,
          TEST_APP_SETTINGS.enterpriseID,
          TEST_APP_SETTINGS_CONFIG,
          tokenManagerFake
        ),
        'Enterprise id should match the parameter value'
  ...
  describe('getAuthorizeURL', function () {
    ...
    it('should return URL for authorize page when called', function () {
      ...
      assert.equal(
        url,
        'https://account.box.com/api/oauth2/authorize?response_type=code&redirect_url=https%3A%2F%2Fexample.com%2F&state=foo&client_id=myId'
      );
    });
  });
  ...
  describe('getTokensAuthorizationCodeGrant()', function () {
    ...
    it('should call to token manager getTokensAuthorizationCodeGrant() and propagate result when called', function (done) {
      var ac = 'ac',
        ...
        fakeTokenInfo = { at: 'at' };
      sandbox
        .mock(tokenManagerFake)
        .expects('getTokensAuthorizationCodeGrant')
