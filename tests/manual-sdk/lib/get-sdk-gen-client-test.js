...
describe('getSdkGenClient()', function () {
  ...
  describe('SDK-Gen client structure', function () {
    it('should have expected resource managers', function () {
      var config = new Config(defaultParams);
      var session = new BasicAPISession('test-token', tokenManagerFake);
      ...
      // Verify common managers exist
      assert.isObject(sdkGenClient.users);
      assert.isObject(sdkGenClient.folders);
      assert.isObject(sdkGenClient.files);
      assert.isObject(sdkGenClient.search);
      assert.isObject(sdkGenClient.comments);
    });

    it('should have expected methods on managers', function () {
      var config = new Config(defaultParams);
      var session = new BasicAPISession('test-token', tokenManagerFake);
      ...
      assert.isFunction(sdkGenClient.users.getUserMe);
      assert.isFunction(sdkGenClient.folders.getFolderById);
      assert.isFunction(sdkGenClient.folders.getFolderItems);
      assert.isFunction(sdkGenClient.files.getFileById);
    });
  });

  describe('Different session types produce working clients', function () {
    it('should work with OAuth session (PersistentAPISession)', function () {
      var config = new Config(defaultParams);
      var tokenInfo = {
    ...
    it('should work with JWT session (AppAuthSession)', function () {
      var config = new Config(jwtParams);
      var session = new AppAuthSession(
        'enterprise',
        'ent123',
    ...
    it('should work with CCG session (CCGAPISession)', function () {
      var config = new Config(ccgParams);
      var session = new CCGAPISession(config, tokenManagerFake);
