...
describe('ChunkedUploader', function () {
  ...
  describe('constructor', function () {
    it('should create an instance of EventEmitter when called', function () {
      let uploader2 = new ChunkedUploader(
        boxClientFake,
        TEST_UPLOAD_SESSION_INFO,
        TEST_FILE,
        TEST_FILE.size
  ...
  describe('start', function () {
    describe('with whole file buffered', function () {
      it('should upload all chunks and commit when the file fits into the initial pieces', function (done) {
        ...
        filesClientMock
          .expects('uploadPart')
          .withArgs(TEST_SESSION_ID, 'abcdefghij', 0, 36)
          .yieldsAsync(null, { part: expectedChunks[0] });
        filesClientMock
          .expects('uploadPart')
          .withArgs(TEST_SESSION_ID, 'klmnopqrst', 10, 36)
          .yieldsAsync(null, { part: expectedChunks[1] });
        filesClientMock
          .expects('uploadPart')
          .withArgs(TEST_SESSION_ID, 'uvwxyz0123', 20, 36)
          .yieldsAsync(null, { part: expectedChunks[2] });
        filesClientMock
          .expects('uploadPart')
          .withArgs(TEST_SESSION_ID, '456789', 30, 36)
          .yieldsAsync(null, { part: expectedChunks[3] });

        filesClientMock
          .expects('commitUploadSession')
          .withArgs(TEST_SESSION_ID, TEST_HASH, {
            parts: sinon.match(expectedChunks),
          })
      ...
      it('should upload all chunks and commit when the file fits into fewer than the initial number of chunks', function (done) {
        ...
        filesClientMock
          .expects('uploadPart')
          .withArgs(TEST_SESSION_ID, 'abcdefghij', 0, 16)
          .yieldsAsync(null, { part: expectedChunks[0] });
        filesClientMock
          .expects('uploadPart')
          .withArgs(TEST_SESSION_ID, 'klmnop', 10, 16)
          .yieldsAsync(null, { part: expectedChunks[1] });

        filesClientMock
          .expects('commitUploadSession')
          .withArgs(TEST_SESSION_ID, hash, {
            parts: sinon.match(expectedChunks),
          })
          .yieldsAsync(null, createdFile);
      ...
      it('should upload all chunks and commit when the file is larger than the initial chunks', function (done) {
        let largeFile =
            'part1 str.part2 str.part3 str.part4 str.part5 str.part6 str',
        ...
        filesClientMock
          .expects('uploadPart')
          .withArgs(TEST_SESSION_ID, 'part1 str.', 0, 59)
          .yieldsAsync(null, { part: expectedChunks[0] });
        filesClientMock
          .expects('uploadPart')
          .withArgs(TEST_SESSION_ID, 'part2 str.', 10, 59)
          .yieldsAsync(null, { part: expectedChunks[1] });
        filesClientMock
          .expects('uploadPart')
          .withArgs(TEST_SESSION_ID, 'part3 str.', 20, 59)
          .yieldsAsync(null, { part: expectedChunks[2] });
        filesClientMock
          .expects('uploadPart')
          .withArgs(TEST_SESSION_ID, 'part4 str.', 30, 59)
          .yieldsAsync(null, { part: expectedChunks[3] });
        filesClientMock
          .expects('uploadPart')
          .withArgs(TEST_SESSION_ID, 'part5 str.', 40, 59)
          .yieldsAsync(null, { part: expectedChunks[4] });
        filesClientMock
          .expects('uploadPart')
          .withArgs(TEST_SESSION_ID, 'part6 str', 50, 59)
      ...
      it('should upload all chunks and commit when the file fits exactly in chunk boundaries', function (done) {
        ...
        filesClientMock
          .expects('uploadPart')
          .withArgs(TEST_SESSION_ID, smallFile, 0, 10)
          .yieldsAsync(null, { part: expectedChunks[0] });
    ...
    it('should resolve to the created file when the file is successfully uploaded', function () {
      ...
      return uploader.start().then((data) => {
      ...
      return uploader
        ...
        .catch((err) => {
          assert.equal(
            err,
            commitError,
            'Promise shoudl reject with commit error'
          );
          assert.equal(
            emittedError,
            commitError,
            'Commit error should have been emitted by uploader'
          );
        });
    });
  });
  ...
  describe('abort()', function () {
    it('should remove event listeners and cancel each existing chunk when called', function (done) {
      ...
      uploader.on('chunkUploaded', () =>
        assert.fail('Should not listen for chunk uploaded event')
      );
      uploader.on('chunkError', () =>
        assert.fail('Should not listen for chunk error event')
      );
      uploader.on('aborted', () => {
      ...
      filesClientMock
        .expects('abortUploadSession')
        .withArgs(TEST_SESSION_ID)
    ...
    it('should emit abortFailed event when abort fails', function (done) {
      ...
      uploader.on('aborted', () =>
        assert.fail('Should not emit aborted event when abort fails')
      );
