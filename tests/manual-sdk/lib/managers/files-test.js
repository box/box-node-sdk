...
describe('Files', function () {
  ...
  describe('getDownloadURL()', function () {
    ...
    it('should return a promise resolving to the download URL when a 302 FOUND response is returned', function () {
      ...
      sandbox.stub(boxClientFake, 'get').returns(Promise.resolve(response));
      return files.getDownloadURL(FILE_ID, testQS).then((location) => {
        assert.strictEqual(
          location,
          response.headers.location,
          'location header is returned'
    ...
    it('should return a promise that rejects when a 202 ACCEPTED response is returned', function () {
      ...
      sandbox.stub(boxClientFake, 'get').returns(Promise.resolve(response));
      return files.getDownloadURL(FILE_ID, testQS).catch((err) => {
        assert.instanceOf(err, Error);
        assert.strictEqual(err.statusCode, response.statusCode);
      });
    });
    ...
    it('should call callback with unexpected response error when the API returns unknown status code', function () {
      ...
      sandbox.stub(boxClientFake, 'get').returns(Promise.resolve(response));
      files.getDownloadURL(FILE_ID, testQS).catch((err) => {
        assert.instanceOf(err, Error);
        assert.strictEqual(err.statusCode, response.statusCode);
      });
    });
  ...
  describe('getReadStream()', function () {
    ...
    it('should work when no options are passed', function () {
      ...
      return files.getReadStream(FILE_ID, null).then((data) => {
    ...
    it('should return promise resolving to the read stream when callback is passed', function () {
      ...
      sandbox
        ...
        .returns(Promise.resolve(downloadURL));
      sandbox.stub(boxClientFake, 'get').returns(Promise.resolve(stream));
      return files.getReadStream(FILE_ID, testQS).then((data) => {
        assert.equal(data, stream);
      });
    });
  });
  ...
  describe('addToCollection()', function () {
    ...
    it('should get current collections and add new collection when item has no collections', function (done) {
      ...
      filesMock
        .expects('get')
        .withArgs(FILE_ID, { fields: 'collections' })
        .returns(Promise.resolve(file));
      filesMock
        .expects('update')
        .withArgs(FILE_ID, { collections: [{ id: COLLECTION_ID }] })
        .returns(Promise.resolve(file));
    ...
    it('should get current collections and add new collection when item has other collections', function (done) {
      ...
      filesMock
        .expects('update')
        .withArgs(FILE_ID, {
          collections: [{ id: '111' }, { id: COLLECTION_ID }],
        })
        .returns(Promise.resolve(file));
    ...
    it('should return promise resolving to the updated file when API calls succeed', function () {
      ...
      return files.addToCollection(FILE_ID, COLLECTION_ID).then((data) => {
        assert.equal(data, file);
      });
    });
    ...
    it('should return promise that rejects when getting current collections fails', function () {
      ...
      return files.addToCollection(FILE_ID, COLLECTION_ID).catch((err) => {
        assert.equal(err, error);
      });
    });
    ...
    it('should call callback with error when adding the collection fails', function (done) {
      ...
      filesMock
        ...
        .withArgs(FILE_ID, { fields: 'collections' })
        .returns(Promise.resolve(file));
      filesMock.expects('update').withArgs(FILE_ID, expectedBody).returns(p);
  ...
  describe('removeFromCollection()', function () {
    ...
    it('should get current collections and pass filtered array when item is in multiple collections', function (done) {
      ...
      filesMock
        .expects('update')
        .withArgs(FILE_ID, { collections: [{ id: '111' }] })
        .returns(Promise.resolve(file));
    ...
    it('should return promise resolving to the updated file when API calls succeed', function () {
      ...
      return files.removeFromCollection(FILE_ID, COLLECTION_ID).then((data) => {
        assert.equal(data, file);
      });
    });
    ...
    it('should return promise that rejects when adding the collection fails', function () {
      ...
      return files.removeFromCollection(FILE_ID, COLLECTION_ID).catch((err) => {
        assert.equal(err, error);
      });
    });
  });
  ...
  describe('delete()', function () {
    it('should make DELETE request to update file info when called', function () {
      sandbox.stub(boxClientFake, 'wrapWithDefaultHandler').returnsArg(0);
      sandbox.mock(boxClientFake).expects('del').withArgs('/files/1234', {});
    ...
    it('should return promise resolving to results when called', function () {
      ...
      return files.delete(FILE_ID).then((data) => assert.equal(data, response));
    });
  });

  describe('prelightUploadFile()', function () {
    var parentFolderID, fileData, uploadsQS, uploadsBody, expectedParams;
    ...
    it('should return promise resolving to results when called', function () {
      ...
      return files
        .preflightUploadFile(parentFolderID, fileData, uploadsQS)
        .then((data) => assert.equal(data, response));
    });
  });

  describe('preflightUploadNewFileVersion()', function () {
    var fileID, fileData, uploadsQS, expectedParams;
    ...
    it('should pass results to callback when callback is present', function (done) {
      ...
      sandbox.stub(boxClientFake, 'wrapWithDefaultHandler').returnsArg(0);
      sandbox.stub(boxClientFake, 'options').yieldsAsync(null, response);
      files.preflightUploadNewFileVersion(
        fileID,
        fileData,
        uploadsQS,
        function (err, data) {
    ...
    it('should return promise resolving to results when called', function () {
      ...
      sandbox.stub(boxClientFake, 'options').returns(Promise.resolve(response));
      return files
        .preflightUploadNewFileVersion(fileID, fileData, uploadsQS)
        .then((data) => assert.equal(data, response));
    });
  });
  ...
  describe('promoteVersion()', function () {
    ...
    it('should return promise resolving to results when called', function () {
      ...
      sandbox.stub(boxClientFake, 'post').returns(Promise.resolve(response));
      return files
        .promoteVersion(FILE_ID, FILE_VERSION_ID)
        .then((data) => assert.equal(data, response));
    });
  });
  ...
  describe('uploadFile()', function () {
    ...
    it('should pass content length parameter when specified', function () {
      ...
      var expectedFormData = {
        ...
        content: {
          ...
          options: {
            filename: 'unused',
            knownLength: options.content_length,
          },
        },
      };
    ...
    it('should return promise resolving to results when called', function () {
      ...
      return files
        .uploadFile(PARENT_FOLDER_ID, FILENAME, CONTENT)
        .then((data) => assert.equal(data, response));
    });
  });
  ...
  describe('uploadNewFileVersion()', function () {
    ...
    it('should return promise resolving to results when called', function () {
      ...
      sandbox.stub(boxClientFake, 'upload').returns(Promise.resolve(response));
      return files
        .uploadNewFileVersion(FILE_ID, CONTENT)
        .then((data) => assert.equal(data, response));
    });
  });
  ...
  describe('getMetadata()', function () {
    ...
    it('should return promise resolving to results when called', function () {
      ...
      sandbox.stub(boxClientFake, 'get').returns(Promise.resolve(response));
      return files
        .getMetadata(FILE_ID, 'global', 'properties')
        .then((data) => assert.equal(data, response));
    });
  });

  describe('addMetadata()', function () {
    var metadata, expectedParams;
    ...
    it('should return promise resolving to results when called', function () {
      ...
      sandbox.stub(boxClientFake, 'post').returns(Promise.resolve(response));
      return files
        .addMetadata(FILE_ID, 'global', 'properties', metadata)
        .then((data) => assert.equal(data, response));
    });
  });

  describe('updateMetadata()', function () {
    var patch, expectedParams;
    ...
    it('should return promise resolving to results when called', function () {
      ...
      sandbox.stub(boxClientFake, 'put').returns(Promise.resolve(response));
      return files
        .updateMetadata(FILE_ID, 'global', 'properties', patch)
        .then((data) => assert.equal(data, response));
    });
  });
  ...
  describe('deleteMetadata()', function () {
    ...
    it('should return promise resolving to results when called', function () {
      ...
      sandbox.stub(boxClientFake, 'del').returns(Promise.resolve(response));
      return files
        .deleteMetadata(FILE_ID, 'global', 'properties')
        .then((data) => assert.equal(data, response));
    });
  });
  ...
  describe('getEmbedLink()', function () {
    ...
    it('should make GET request to create embed link', function () {
      var embedLink = {
          expiring_embed_link: {
            url: 'https://app.box.com/preview/expiring_embed/1234',
          },
        },
    ...
    it('should call callback with the embed link when a 200 ok response is returned', function (done) {
      ...
      sandbox
        .stub(boxClientFake, 'get')
        .withArgs(`/files/${FILE_ID}`)
        .returns(Promise.resolve(response));
    ...
    it('should return promise resolving to the embed link when a 200 ok response is returned', function () {
      ...
      sandbox
        ...
        .withArgs(`/files/${FILE_ID}`)
        .returns(Promise.resolve(response));
      return files.getEmbedLink(FILE_ID).then((data) => {
        assert.equal(data, embedLink.expiring_embed_link.url);
      });
    });
    ...
    it('should return promise that rejects with a response error when API returns non-200 result', function () {
      ...
      return files.getEmbedLink(FILE_ID).catch((err) => {
        assert.instanceOf(err, Error);
        assert.propertyVal(err, 'statusCode', response.statusCode);
      });
    });
  ...
  describe('lock()', function () {
    var expiresAt, isDownloadPrevented, expectedParams;
    ...
    it('should return promise resolving to results when called', function () {
      ...
      sandbox.stub(boxClientFake, 'wrapWithDefaultHandler').returnsArg(0);
      sandbox.stub(boxClientFake, 'put').returns(Promise.resolve(response));
      return files.lock(FILE_ID).then((data) => assert.equal(data, response));
    });
  });
  ...
  describe('unlock()', function () {
    ...
    it('should return promise resolving to results when called', function () {
      ...
      sandbox.stub(boxClientFake, 'wrapWithDefaultHandler').returnsArg(0);
      sandbox.stub(boxClientFake, 'put').returns(Promise.resolve(response));
      return files.unlock(FILE_ID).then((data) => assert.equal(data, response));
    });
  });
  ...
  describe('getWatermark()', function () {
    ...
    it('should return promise that rejects when API call returns error', function () {
      ...
      sandbox
        ...
        .withArgs(`/files/${FILE_ID}/watermark`)
        .returns(Promise.reject(apiError));
      return files.getWatermark(FILE_ID).catch((err) => {
        assert.equal(err, apiError);
      });
    });
    ...
    it('should return promise resolving to watermark data when API call succeeds', function () {
      ...
      sandbox
        ...
        .withArgs(`/files/${FILE_ID}/watermark`)
        .returns(Promise.resolve(res));
      return files.getWatermark(FILE_ID).then((data) => {
        assert.equal(data, watermark);
      });
    });
  });
  ...
  describe('deleteVersion()', function () {
    ...
    it('should return promise resolving to results when called', function () {
      ...
      sandbox.stub(boxClientFake, 'del').returns(Promise.resolve(response));
      return files
        .deleteVersion(FILE_ID, FILE_VERSION_ID)
        .then((data) => assert.equal(data, response));
    });
  });
  ...
  describe('createUploadSession', function () {
    ...
    it('should make POST request to create a new upload session', function () {
      ...
      sandbox
        ...
        .expects('post')
        .withArgs(
          'https://upload-base/2.1/files/upload_sessions',
          expectedParams
        );
    ...
    it('should return promise resolving to results when called', function () {
      ...
      sandbox.stub(boxClientFake, 'post').returns(Promise.resolve(response));
      return files
        .createUploadSession(TEST_FOLDER_ID, TEST_SIZE, TEST_NAME)
        .then((data) => assert.equal(data, response));
    });
  });
  ...
  describe('createNewVersionUploadSession', function () {
    ...
    it('should make POST request to create a new upload session', function () {
      ...
      sandbox
        ...
        .expects('post')
        .withArgs(
          `https://upload-base/2.1/files/${FILE_ID}/upload_sessions`,
          expectedParams
        );
    ...
    it('should pass results to callback when callback is present', function (done) {
      ...
      sandbox.stub(boxClientFake, 'wrapWithDefaultHandler').returnsArg(0);
      sandbox.stub(boxClientFake, 'post').yieldsAsync(null, response);
      files.createNewVersionUploadSession(
        FILE_ID,
        TEST_SIZE,
        function (err, data) {
          assert.ifError(err);
    ...
    it('should return promise resolving to results when called', function () {
      ...
      sandbox.stub(boxClientFake, 'post').returns(Promise.resolve(response));
      return files
        .createNewVersionUploadSession(FILE_ID, TEST_SIZE)
        .then((data) => assert.equal(data, response));
    });
  });
  ...
  describe('uploadPart', function () {
    ...
    it('should make PUT call to upload part when called', function () {
      var apiResponse = {
        statusCode: 200,
        body: new Buffer(
          '{"part": {"part_id": "00000000", "size": 10, "offset": 0, "sha1": "0987654321abcdef"}}'
        ),
      };
      ...
      sandbox
        ...
        .expects('put')
        .withArgs(
          `https://upload-base/2.1/files/upload_sessions/${TEST_SESSION_ID}`,
          expectedParams
        )
    ...
    it('should return promise that rejects when the API call fails', function () {
      ...
      return files
        .uploadPart(TEST_SESSION_ID, TEST_PART, TEST_OFFSET, TEST_LENGTH)
        .catch((err) => {
          assert.equal(err, error);
        });
    });
  ...
  describe('commitUploadSession', function () {
    ...
    it('should make POST request to commit the upload session when called with list of parts', function () {
      ...
      sandbox
        ...
        .expects('post')
        .withArgs(
          `https://upload-base/2.1/files/upload_sessions/${TEST_SESSION_ID}/commit`,
          expectedParams
        )
        .returns(Promise.resolve(response));
      files.commitUploadSession(TEST_SESSION_ID, TEST_FILE_HASH, {
        parts: TEST_PARTS,
      });
    });
    ...
    it('should return promise that rejects when there is an error making the API call', function () {
      ...
      sandbox.stub(boxClientFake, 'post').returns(p);
      return files
        .commitUploadSession(TEST_SESSION_ID, TEST_FILE_HASH, {
          parts: TEST_PARTS,
        })
        .catch((err) => {
          assert.equal(err, error);
    ...
    it('should retry the call when the API returns a 202 with Retry-After header', function (done) {
      ...
      assert.equal(
        apiStub.callCount,
        1,
        'Retry should not be called until retry interval has elapsed'
      );
    ...
    it('should commit with all parts when API parts span multiple pages', function (done) {
      ...
      filesMock
        .expects('getUploadSessionParts')
        .withArgs(TEST_SESSION_ID, sinon.match({ limit: 1000 }))
        .returns(
          Promise.resolve({
            entries: [parts[0]],
            limit: 1,
      ...
      filesMock
        .expects('getUploadSessionParts')
        .withArgs(TEST_SESSION_ID, sinon.match({ limit: 1000, offset: 1 }))
        .returns(
          Promise.resolve({
            entries: [parts[1]],
            limit: 1,
    ...
    it('should call callback with an error when page request returns an error', function (done) {
      ...
      sandbox
        .stub(files, 'getUploadSessionParts')
        .returns(Promise.reject(partsError));
  ...
  describe('abortUploadSession', function () {
    ...
    it('should return promise resolving to results when called', function () {
      ...
      sandbox.stub(boxClientFake, 'del').returns(Promise.resolve(response));
      return files
        .abortUploadSession(TEST_SESSION_ID)
        .then((data) => assert.equal(data, response));
    });
  });
  ...
  describe('getUploadSessionParts', function () {
    ...
    it('should make GET request for the uploaded parts when called', function () {
      ...
      sandbox
        ...
        .expects('get')
        .withArgs(
          `https://upload-base/2.1/files/upload_sessions/${TEST_SESSION_ID}/parts`,
          testParamsWithQs
        );
    ...
    it('should return promise resolving to results when called', function () {
      ...
      sandbox.stub(boxClientFake, 'get').returns(Promise.resolve(response));
      return files
        .getUploadSessionParts(TEST_SESSION_ID, testQS)
        .then((data) => assert.equal(data, response));
    });
  });
  ...
  describe('getUploadSession', function () {
    ...
    it('should return promise resolving to results when called', function () {
      ...
      sandbox.stub(boxClientFake, 'get').returns(Promise.resolve(response));
      return files
        .getUploadSession(TEST_SESSION_ID)
        .then((data) => assert.equal(data, response));
    });
  });
  ...
  describe('getChunkedUploader', function () {
    ...
    it('should call callback with new chunked uploader when upload session is created', function (done) {
      ...
      sandbox
        .stub(files, 'createUploadSession')
        .returns(Promise.resolve(session));
      files.getChunkedUploader(
        TEST_FOLDER_ID,
        TEST_SIZE,
        ...
        function (err, data) {
          assert.ifError(err);
          assert.ok(
            ChunkedUploaderStub.calledWithNew(),
            'New chunked uploader should be constructed'
          );
          assert.ok(
            ChunkedUploaderStub.calledWith(
              boxClientFake,
              session,
              'test data',
              TEST_SIZE,
            ...
            'Chunked uploader should get correct options'
          );
  ...
  describe('getNewVersionChunkedUploader', function () {
    ...
    it('should make call to create upload session when called', function () {
      ...
      sandbox
        .mock(files)
        .expects('createNewVersionUploadSession')
        .withArgs(TEST_FILE_ID, TEST_SIZE)
        .returns(Promise.resolve(session));
      files.getNewVersionChunkedUploader(
        TEST_FILE_ID,
        TEST_SIZE,
        'test data',
        {}
    ...
    it('should call callback with an error when upload session cannot be created', function (done) {
      ...
      sandbox
        .stub(files, 'createNewVersionUploadSession')
        .returns(Promise.reject(error));
      files.getNewVersionChunkedUploader(
        TEST_FILE_ID,
        TEST_SIZE,
    ...
    it('should return promisethat rejects  when upload session cannot be created', function () {
      ...
      sandbox
        ...
        .returns(Promise.reject(error));
      return files
        .getNewVersionChunkedUploader(TEST_FILE_ID, TEST_SIZE, 'test data', {})
        .catch((err) => {
          assert.equal(err, error);
        });
    });
  ...
  describe('getCollaborations()', function () {
    ...
    it('should return promise resolving to results when called', function () {
      ...
      sandbox.stub(boxClientFake, 'get').returns(Promise.resolve(response));
      return files
        .getCollaborations(FILE_ID, testQS)
        .then((data) => assert.equal(data, response));
    });
  });
  ...
  describe('getRepresentationInfo()', function () {
    ...
    var expectedRepresentationParam = {
      headers: { 'x-rep-hints': TEST_REPRESENTATION },
      qs: { fields: 'representations' },
    };
    ...
    it('should call callback with the representation when a 200 response is returned', function (done) {
      ...
      sandbox.stub(boxClientFake, 'get').returns(Promise.resolve(response));
      files.getRepresentationInfo(
        FILE_ID,
        TEST_REPRESENTATION,
        function (err, representationObject) {
          assert.ifError(err);
          assert.strictEqual(
            representationObject,
            response.body.representations,
            'representation object is returned'
          );
          done();
        }
      );
    ...
    it('should return a promise resolving to a representation object when a 200 response is returned', function () {
      ...
      sandbox.stub(boxClientFake, 'get').returns(Promise.resolve(response));
      return files
        .getRepresentationInfo(FILE_ID, TEST_REPRESENTATION)
  ...
  describe('getRepresentationContent()', function () {
    it('should reject with error when representation info is empty', function () {
      ...
      sandbox
        .mock(files)
        .expects('getRepresentationInfo')
        .withArgs(fileID, representation)
        .returns(Promise.resolve(repsInfo));

      return files
        .getRepresentationContent(fileID, representation)
        .catch((err) => {
          assert.instanceOf(err, Error);
        });
    });
    ...
    it('should reject with error when representation has error state', function () {
      ...
      sandbox
        .stub(files, 'getRepresentationInfo')
        .returns(Promise.resolve(repsInfo));
    ...
    it('should make call to start asset download and resolve to download stream when representation has success state', function () {
      ...
      return files
        .getRepresentationContent(fileID, representation, { assetPath })
        .then((value) => {
