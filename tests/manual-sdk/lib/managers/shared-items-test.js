...
describe('SharedItems', function () {
  ...
  describe('get()', function () {
    var testAuthHeader, expectedParams;
    ...
    it('should make GET request to get shared item info when called', function () {
      ...
      sandbox
        .stub(boxClientFake, 'buildSharedItemAuthHeader')
        .returns(testAuthHeader);
      sandbox
        .mock(boxClientFake)
        .expects('get')
    ...
    it('should return promise resolving to the shared item info when a 200 OK response is returned', function () {
      ...
      return sharedItems
        .get(testSharedItemURL, testSharedItemPassword, testQS)
        .then((data) => {
          assert.equal(data, responseBody);
        });
    });
    ...
    it('should call callback with a password missing error when a password is given and a 403 FORBIDDEN response is returned', function (done) {
      ...
      sharedItems.get(testSharedItemURL, null, testQS, function (err) {
        ...
        assert.propertyVal(
          err,
          'message',
          'Shared link password missing [403 Forbidden]'
        );
    ...
    it('should return promise rejecting with a password_missing error when a password is given and a 403 FORBIDDEN response is returned', function () {
      ...
      sandbox
        ...
        .returns(testAuthHeader);
      sandbox.stub(boxClientFake, 'get').returns(Promise.resolve(response));
      return sharedItems.get(testSharedItemURL, null, testQS).catch((err) => {
        assert.instanceOf(err, Error);
        assert.propertyVal(
          err,
          'message',
      ...
      sharedItems.get(
        ...
        function (err) {
          ...
          assert.propertyVal(
            err,
            'message',
            'Incorrect shared link password [403 Forbidden]'
          );
          assert.propertyVal(err, 'statusCode', response.statusCode);
          done();
        }
