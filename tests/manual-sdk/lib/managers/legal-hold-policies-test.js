...
describe('LegalHoldPolicies', function () {
  beforeEach(function () {
    ...
    LegalHoldPolicies = require(MODULE_FILE_PATH).default;
  ...
  describe('getAssignment', function () {
    ...
    it('should pass results to callback when callback is present', function (done) {
      ...
      sandbox.stub(boxClientFake, 'wrapWithDefaultHandler').returnsArg(0);
      sandbox.stub(boxClientFake, 'get').yieldsAsync(null, response);
      legalHoldPolicies.getAssignment(
        ASSIGNMENT_ID,
        null,
        function (err, data) {
          assert.ifError(err);
  ...
  describe('getFileVersionLegalHold()', function () {
    ...
    it('should pass results to callback when callback is present', function (done) {
      ...
      sandbox.stub(boxClientFake, 'wrapWithDefaultHandler').returnsArg(0);
      sandbox.stub(boxClientFake, 'get').yieldsAsync(null, response);
      legalHoldPolicies.getFileVersionLegalHold(
        LEGAL_HOLD_ID,
        null,
        function (err, data) {
          assert.ifError(err);
    ...
    it('should return promise resolving to results when called', function () {
      ...
      sandbox.stub(boxClientFake, 'get').returns(Promise.resolve(response));
      return legalHoldPolicies
        .getFileVersionLegalHold(LEGAL_HOLD_ID)
        .then((data) => assert.equal(data, response));
    });
  });
  ...
  describe('getAllFileVersionLegalHolds()', function () {
    ...
    it('should pass results to callback when callback is present', function (done) {
      ...
      sandbox.stub(boxClientFake, 'wrapWithDefaultHandler').returnsArg(0);
      sandbox.stub(boxClientFake, 'get').yieldsAsync(null, response);
      legalHoldPolicies.getAllFileVersionLegalHolds(
        POLICY_ID,
        null,
        function (err, data) {
          assert.ifError(err);
    ...
    it('should return promise resolving to results when called', function () {
      ...
      sandbox.stub(boxClientFake, 'get').returns(Promise.resolve(response));
      return legalHoldPolicies
        .getAllFileVersionLegalHolds(POLICY_ID)
        .then((data) => assert.equal(data, response));
    });
  });
});
