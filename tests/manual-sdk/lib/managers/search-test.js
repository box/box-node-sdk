...
describe('Search', function () {
  ...
  beforeEach(function () {
    ...
    SearchManager = require(MODULE_FILE_PATH).default;
  ...
  describe('query()', function () {
    var searchQuery, fakeQs, fakeParamsWithQs;
    ...
    it('should properly encode metadata filters when called with mdfilters option', function () {
      ...
      var expectedParams = {
        qs: {
          mdfilters:
            '[{"templateKey":"MyTemplate","scope":"enterprise","filters":{}}]',
          query: '',
        },
      };
    ...
    it('should return promise that rejects when mdfilters is invalid', function () {
      ...
      sandbox.mock(boxClientFake).expects('get').never();
      return search.query('', options).catch((err) => {
        assert.instanceOf(err, Error);
      });
    });
  });
