	...
	describe('Retention Policies', function() {
		...
		describe('getFilesVersionUnderRetentionForAssignment()', function() {
			it('should make GET call to retrieve file versions under retention for assignment', function() {
				...
				apiMock.get(`/2.0/retention_policy_assignments/${assignmentID}/files_versions_under_retention`)
					.matchHeader('Authorization', function(authHeader) {
						assert.equal(authHeader, `Bearer ${TEST_ACCESS_TOKEN}`);
						return true;
					})
				...
				return basicClient.retentionPolicies.getFilesVersionUnderRetentionForAssignment(assignmentID)
					.then(files => assert.deepEqual(files, JSON.parse(fixture)));
			});
		});
	});
